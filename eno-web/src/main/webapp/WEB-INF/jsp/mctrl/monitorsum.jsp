                    ...
                    <div>
                        <div class="energy_chart">
                            <div class="chart_bottom">
                                ...
                                <div class="chart_bottom_data"><span class="data_value">{{Exp_ENVMS_jrznh}}</span><span class="num_unit">kWh</span></div>
                                <span class="chart_bottom_title">当前用电功率</span>
                                <div class="chart_bottom_data"><span class="data_value">{{Exp_ENVMS_dqydgl}}</span><span class="num_unit">kW</span></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
	...
	function monitorSumController($scope, $http) {
		...
		$scope.Exp_FAS_qt = 4;//其他
		$scope.Exp_ENVMS_jrznh = 5;//今日总能耗
		$scope.Exp_ENVMS_dqydgl = 6;//当前用电功率

		$scope.FAS_sdbj=3334;
		$scope.FAS_qt=4444;
		$scope.Exp_ENVMS_jrznh=100;
		$scope.Exp_ENVMS_dqydgl=200;
		...
		$http({method: 'GET', url: CONTEXT_PATH + '/msum/getMonitorSumData'}).success(function(data, status, headers){
			for (var i=0; i<data.length; i++){
				var funStr = "$scope."+data[i].name+"='"+data[i].value+"'";
				eval(funStr);
			}
		});
