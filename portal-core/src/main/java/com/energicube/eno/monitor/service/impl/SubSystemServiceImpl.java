...
public class SubSystemServiceImpl implements SubSystemService {
    ...
    public List<Object> findPanelContent(String layoutid, String pagetagid, List<TagInfo> taginfo, Map<String, Dict> contextMap) {
        ...
        return getPanelHtml(layoutid, pagetagList, taginfo, contextMap);
    }
    ...
    public Map<String, List> getDeviceMap(String layoutid, Map<String, List> map, List<Pagetag> pagetagList, List<TagInfo> taginfo, Map<String, Dict> contextMap) {
        if (StringUtils.hasLength(layoutid)) {
            ...
            if (py.getDeviceconfigid() != null) {
                ...
                if (dConfig != null) {
                    String attribute = dConfig.getAttribute(), desc = dConfig.getDescription(); // 设备列表中文名称及英文名称
                    String[] ats = attribute.split(","), ds = desc.split(",");
...
//										try {
//											String tagval = redisOpsService.hGetAllValueByKey(String.format("tags:%s", ma.getValuetag()));
//											valueList.add(tagval != null ? tagval : "0");
//											logger.debug("tagid:[" + ma.getValuetag() + "],tagval:[" + (tagval != null ? tagval : "0") + "]");
//										} catch (Exception e) {
//											valueList.add("0");
...
//										}
                                        ...
                                        try {
                                            ...
                                            tagval = getValueByKey(dConfig.getClassid(), ats[j], tagval, contextMap);
                                            valueList.add(tagval != null ? tagval : "0");
                                        } catch (Exception e) {
                                            valueList.add("0");
                        ...
                        if (asset != null) {
                            ...
                            for (int j = 0; j < notSetList.size(); j++) {
                                ...
                                if (objs != null && objs.length > 0) {
                                    _attr = objs[0].toString();
                                    _desc = objs[1].toString();
                                    _unitid = objs[2].toString();
                                    _classstructureid = objs[3].toString();
                                }
...
//										String value = "0";
//										try {
//											value = redisOpsService.hGetAllValueByKey(String.format("tags:%s", ma.getValuetag()));
//										} catch (Exception e) {
//											value = "0";
//											logger.error("根据tagid获取redis中的值失败了,tagid为[" + ma.getValuetag() + "]");
//										}
                                        ...
                                        try {
                                            ...
                                            value = getValueByKey(_classstructureid, _attr, value, contextMap);
                                        } catch (Exception e) {
                                            value = "0";
                                            logger.error("tagid为【" + ma.getValuetag() + "】的记录获取value失败！");
                                        }
                                        html += "<td class='panelTd_1'>" + ma.getDescrption() + "</td>" + "<td class='p_" + ma.getValuetag() + " color_2 panelTd_2'>" + value + "</td>" + "<td class='panelTd_2'>" + ("——".equals(ma.getMeasureunitid()) ? "" : ma.getMeasureunitid()) + "</td>";
                                        have = true; // true说明找到了对应属性的tagid
                                        break;
                                    }
                                }
                            ...
                            // 查询所有设定值属性记录
                            List<Object> setList = assetMeasurementRepository.findNotSetAttribute(asset.getClassstructureid(), "");
                            for (int j = 0; j < setList.size(); j++) {
                                ...
                                if ("status_sp".equalsIgnoreCase(_attr)) { // 启停控制，显示开关按钮
                                    html += "		<div class='switch_btn'>"
                                            + "			<div class='switch switch-large panel_switch' tagid='" + pt.getTagid() + "'>"
                                            + "				<input type='checkbox' ";
                                    int status_sp = new Random().nextInt(2); // 随机出来1和0，随机显示开关按钮
                                    html += (status_sp == 0 ? "" : "checked") + " />"
                                            + "			</div>"
                                            + "		</div>";
                                }
...
//										}
                                        ...
                                        if (!"status_sp".equals(_attr)) { // 启停设定，不需要增加
                                            html += "<li><span class='param_mode'>" + ma.getDescrption() + "</span><div class='input_div'><input type='text' placeholder='18.0' value='" + value + "' class='p_" + ma.getValuetag() + "' /><span class='param_unit'>" + ("——".equals(ma.getMeasureunitid()) ? "" : ma.getMeasureunitid()) + "</span></div></li>";
                                        }
                                        have = true; // true说明找到了对应属性的tagid
                                        break;
                                    }
     ...
     */
    private String getValueByKey(String classstructureid, String assetattrid, String value, Map<String, Dict> contextMap) {
        String str = value;
        try {
