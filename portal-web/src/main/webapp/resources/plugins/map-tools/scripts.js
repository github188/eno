...
{
	// MapURL接收配置页面传递来的地图URL参数信息 [ ChengKang 2014-07-21 ]
	var Param = window.opener.hotArea;		// 配置页面传递给热区绘制页面的参数结构体 [ ChengKang 2014-07-22 ]
	console.log(window.opener);
	var MapURL = Param.mapUrl;						// 绘制热区的【系统平面图】的底图URL地址 [ ChengKang 2014-07-22 ]
	var LayoutId = Param.layoutid;					// 配置页面的LayoutID  [ ChengKang 2014-07-22 ]
	var PageTagId = Param.pagetagid;				// Tag主键
	...
	// 将配置页面传递来的地图URL直接赋值给url控件
	utils.id("url").value = MapURL;
	...
	var app = (function() {
		...
		return {
			...
			// 单击保存按钮响应  [ ChengKang 2014-07-22 ]
			saveInDb : function()
			{
				// 将绘制的热区生成的HTML转换成数据库需要保存在setting的JSON字符串数组 [ ChengKang 2014-07-23 ]
				var JsonArray = app.getJsonCode();
				// 将JSON字符串数组存储在本地存储对象，供配置页面调用后保存到数据库 [ ChengKang 2014-07-23 ]
				window.localStorage.setItem('MapJsonArray', JsonArray);
			},
			...
			saveInLocalStorage : function() {
				...
				// 单击完成按钮同样会做saveInDb的功能 [ ChengKang 2014-07-23 ]
				// 将绘制的热区生成的HTML转换成数据库需要保存在setting的JSON字符串数组
				// 将JSON字符串数组存储在本地存储对象，供配置页面调用后保存到数据库
				var JsonArray = app.getJsonCode();
				window.localStorage.setItem('MapJsonArray', JsonArray);
				
				var JsonStr = null;
				if(PageTagId != 0)		// 表示修改热区（只处理一个热区）
				{
					// 获取JSON数组中的第一个元素，转换成String，作为参数传递给回调函数Reback [ ChengKang 2014-08-20 ]
					JsonStr =  JSON.stringify( JsonArray[0] );
				}
				else							// 表示新增热区（可以一次新建多个热区）
				{
					JsonStr =  JSON.stringify( JsonArray );
				}
				
				
				// 调用父窗口的方法 [ ChengKang 2014-08-20 ]
				Param.Reback(JsonStr, PageTagId, LayoutId);
				
				// alert('热区创建或编辑成功，请关闭窗口后保存热区！');
			...
			loadFromLocalStorage : function() {
				var str = window.localStorage.getItem('SummerHTMLImageMapCreator'),
					obj = JSON.parse(str),
					ObjArea = obj.areas;
				// 此处不需要再次加载图片，图片在窗口初始化时被加载 [ ChengKang 2014-07-21 ]
				// this.loadImage(obj.img);

				utils.foreach(ObjArea, function(x)
				{
					switch (x.type)
					{
						case 'rect':
			...
			getHTMLCode : function(arg) {
				...
				if (arg) {
					...
					html_code += utils.encode('<img src="' + filename + '" alt="" usemap="#map" />') +
						'<br />' + utils.encode('<map name="map" id="map">') + '<br />';
			...
			// 对绘制的热区HTML代码做转换，转换成符合Json格式的字符串
			getJsonCode : function()
			{
				...
				utils.foreachReverse(objects, function(x)
				{
					...
					html_code = x.toString();																	// 比如，原始的HTML格式为：<area shape="rect" coords="20, 30, 40, 50" TagID="123" TagName="AAA" />
					html_code = html_code.replaceAll("\"", "");										// 去掉所有引号，得到：<area shape=rect coords=20,30,40,50 TagID=123 TagName=AAA />
					html_code = html_code.replaceAll("<area ", "{\"plan\":{");					// 将“<area ”替换为“{"plan":{”得到：{"plan":{shape=rect coords=20,30,40,50  TagID=123 TagName=AAA />
					html_code = html_code.replaceAll(" />", "\"}}");								// 将“ />”替换为“]}}”得到：{"plan":{shape=rect coords=20,30,40,50 TagID=123 TagName=AAA"}}
					html_code = html_code.replaceAll(" TagID=", "] \"TagID\"=\"");		// 将“ TagID=”替换为“] "TagID"="”得到：{"plan":{shape=rect coords=20,30,40,50] "TagID"="123 TagName=AAA"}}
					html_code = html_code.replaceAll(" ", ",");										// 将空格替换为“,”，得到：{"plan":{shape=rect,coords=20,,30,,40,,50],"TagID"="123 ,TagName=AAA"}}
					html_code = html_code.replaceAll(",,", ",");										// 将“,,”替换为“,”，得到：{"plan":{shape=rect,coords=20,30,40,50],"TagID"="123,TagName=AAA"}}
					html_code = html_code.replaceAll("shape", "\"shape\"");					// 将“shape”替换为“"shape"”，得到：{"plan":{"shape"=rect,coords=20,30,40,50],"TagID"="123,TagName=AAA"}}
					html_code = html_code.replaceAll("coords=", "\"coords\"=[");			// 将“coords=”替换为“"coords"=[”，得到：{"plan":{"shape"=rect,"coords"=[20,30,40,50],"TagID"="123,TagName=AAA"}}
					html_code = html_code.replaceAll("rect", "\"rect\"");						// 将“rect”替换为“"rect"”，得到：{"plan":{"shape"="rect","coords"=[20,30,40,50],"TagID"="123,TagName=AAA"}}
					html_code = html_code.replaceAll("circle", "\"circle\"");					// 将“circle”替换为“"circle"”
					html_code = html_code.replaceAll("poly", "\"poly\"");						// 将“poly”替换为“"poly"”
					html_code = html_code.replaceAll("=", ":");										// 将“=”替换为“:”，得到：{"plan":{"shape":"rect","coords":[20,30,40,50],"state":"123,TagName:AAA"}}
					html_code = html_code.replaceAll(",TagName:", "\",\"TagName\":\"");	// 将“,TagName:”替换为“","TagName":[”，得到：{"plan":{"shape":"rect","coords":[20,30,40,50],"state":"123,"TagName":"AAA"}}
					// alert(html_code);
					JsonArray.push(html_code);
				});
				return JsonArray;
			}
	...
	var info = (function() {
		var form = utils.id('edit_details'),
			...
			state_attr = utils.id('state_attr'),
		...
		// 弹出配置热区窗口上的保存按钮事件 [ ChengKang 2014-07-22 ]
		function save(e) {

			obj.state = state_attr.value;
			...
			document.all("edit").click();		// 在新建一个热区后，自动弹出热区配置窗口，此处点击保存按钮时，自动退出编辑状态
			utils.hide(form);
		};
		...
		state_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
		...
		state_attr.addEventListener('change', change, false);
		...
		return {
			load : function(object, new_x, new_y) {
				obj = object;
				state_attr.value = object.state ? object.state : '';
		...
		function test(str) {
			...
			if (str) {
				...
				while (result_area) {
					...
					regexp_state = regexp_state.exec(area);
					if (result_state) {
						href = result_href[1];
					} else {
						href = '';
	...
	var get_image = (function() {
		...
		var url_input = (function() {
			...
			return {
				clear : clearUrl,
				init : function() {
					// 将此处clear()调用注释掉，直接在URL输入框显示配置页面传递来的地图URL地址 [ ChengKang 2014-07-21 ]
					// this.clear();
					utils.hide(url_clear_button);
				},
				test : function() {
	...
	var buttons = (function() {
		...
		function onSaveDbButtonClick(e) {
			// Save in Db 即：保存按钮
			app.saveInDb();
		...
		save_db.addEventListener('click', onSaveDbButtonClick, false);
	...
	Rect.prototype.onDrawStop = function(e) {
		...
		// 绘制一个新的热区后，自动弹出热区配置窗口 [ ChengKang 2014-07-22 ]
		document.all("edit").click();
		info.load(_n_f, e.pageX, e.pageY);
	};
	...
	// 将热区信息转换为HTML格式代码 [ ChengKang 2014-07-22 ]
	Rect.prototype.toString = function() { //to html map area code
		...
		return '<area shape="rect" coords="'
			...
			+ y2
			+ '"'
			+ (this.state ? ' TagID="' + this.state + '"' : '')
			+ (this.href ? ' TagName="' + this.href + '"' : '')
	...
	var Polygon = function(x, y){
		...
		this.state = ''; //state arrtibute - not required
		this.href = ''; //href attribute - not required
		this.alt = ''; //alt attribute - not required
		this.title = ''; //title attribute - not required
	...
	// 自动触发OK按钮的click事件，加载图片 [ChengKang 2014-07-21]
	function PutMapImage()
	{
		document.all("button").click();
		document.all("load").click();
	};
	PutMapImage();
};
...
else if (/MSIE/i.test(navigator.userAgent))	// 如果是 IE 浏览器
{
	...
	script.onreadystatechange = function()
	{
		...
		{
			SummerHtmlImageMapCreator();
		}
	}
}
else if (/WebKit/i.test(navigator.userAgent))		// 如果是 Safari 浏览器
{
